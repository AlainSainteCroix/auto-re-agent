...
class SourceIndexer:
    ...
    def __init__(self, source_root: Path, profile: ProjectProfile | None = None) -> None:
        ...
        self.source_files: list[Path] = self._discover_source_files(extensions)
    ...
    def _discover_source_files(self, extensions: list[str]) -> list[Path]:
        """Collect readable source files under ``source_root``.

        Defensive against a missing/non-directory root, permission errors during
        traversal, and entries that match ``*.ext`` but are not regular files
        (directories named like sources, broken symlinks). Such entries are
        skipped rather than allowed to abort indexing.
        """
        found: set[Path] = set()
        for ext in extensions:
            try:
                for p in self.source_root.rglob(f"*{ext}"):
    ...
    def _read_text(self, path: Path) -> str:
        ...
        if txt is None:
            try:
                ...
                # File vanished, permission denied, or not a regular file:
                # treat as empty so a single bad path can't abort indexing
                # or a later lookup.
                txt = ""
